...
// Interrupt mask in the CPLD
static UINT8 sg_u8InterruptMask = 0xff;
static UINT8 sg_u8InterruptMask2 = 0xff;
...
//				DebugOut("%s: Character write - 0x%.8x\n", __FUNCTION__, u8Data);
...
// RTC
static UINT8 RTCRead(UINT32 u32Address)
{
	return(0xff);
}

static void RTCWrite(UINT32 u32Address,
					 UINT8 u8Data)
{
}
...
static unsigned char RSC68kDevComRead8(unsigned long u32Address)
{
	...
	// RTC
	if ((u32Address >= RSC68KHW_DEVSPEC_RTC) && (u32Address < RSC68KHW_DEVSPEC_FLASH_DRAM_MAP))
	{
		return(RTCRead((u32Address ^ 1) >> 1));
	}
	GCASSERT(0);
	return(0xff);
}
								   ...
								   unsigned char u8Data)
{
	...
	// Interrupt mask?
	if ((u32Address ^ 1) == RSC68KHW_DEVCOM_INTC_MASK)
	{
		sg_u8InterruptMask = u8Data;
		DebugOut("Interrupt mask: 0x%.2x\n", u8Data);
		return;
	}

	// Interrupt mask2?
	if ((u32Address ^ 1) == RSC68KHW_DEVCOM_INTC_MASK2)
	{
		sg_u8InterruptMask2 = u8Data;
		DebugOut("Interrupt mask 2: 0x%.2x\n", u8Data);
		return;
	}

	// IDE?
	if ((u32Address >= RSC68KHW_DEVCOM_IDE_CSA) && (u32Address < RSC68KHW_DEVCOM_STATUS_LED))
	{
		// Doesn't do anything
		return;
	}

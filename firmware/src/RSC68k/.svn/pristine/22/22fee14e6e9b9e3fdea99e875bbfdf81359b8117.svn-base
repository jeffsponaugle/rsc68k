/*	DIS68K by wrm
	...
	Modified 05/19/2022 for RSC68k project by CNB
	*/
	...
	switch(mode) {
		...
		case 9  : { /* pcr + disp */
			...
			if (mode == 5) {
				...
				if (!rawmode) {
					sprintf(out_s, "%+i(PC) {0x%06u}", displacement, ldata);
				} else {
		...
		case 7  :
			sprintf(out_s, "0x0000%04x", getword());
			break;
		case 8  : {
			const int data1 = getword();
			const int data2 = getword();
			sprintf(out_s, "0x%04x%04x", data1, data2);
		} break;
		case 11 : {
			const int data1 = getword();
			switch(size) {
				case 0 : sprintf(out_s, "#0x%02x", (data1 & 0x00FF));
					break;
				case 1 : sprintf(out_s, "#0x%04x", data1);
					break;
				case 2 : {
					const int data2 = getword();
					sprintf(out_s, "#0x%04x%04x", data1, data2);
...
uint32_t disasm(uint32_t start) {
	address = start;
	char operand_s[100];

	{
			...
			if ((word & optab[opnum].and) == optab[opnum].xor) {
				...
				switch(opnum) { /* opnum = 1..85 */
					...
					case 79 : { /* ADDI, ANDI, CMPI, EORI, ORI, SUBI */
						...
						char source_s[50];
						switch(size) {
							case 0 : sprintf(source_s, "#0x%02X", (data & 0x00FF));
								break;
							case 1 : sprintf(source_s, "#0x%04X", data);
								break;
							case 2 :
								sprintf(source_s, "#0x%04X%04X", data, getword());
								break;
						}
					...
					case 13 : {/* Bcc */
						...
						if (offset != 0) {
							...
							if (!rawmode) {
								sprintf(operand_s, "0x%08x", address + offset);
							} else {
							...
							if (!rawmode) {
								sprintf(operand_s, "0x%08x" , address - 2 + offset);
							} else {
								sprintf(operand_s, "*%+d", offset);
							}
						}
					...
					case 28 : { /* DBcc */
						...
						const int dreg = word & 0x0007;
						sprintf(operand_s, "D%i,0x%08x", dreg, address - 2 + offset);
						decoded = true;
					} break;
					case 33 : { /* EXG */
					...
					case 50 : {/* MOVEP */
						...
						if ((word & 0x0080) == 0) {
							/* mem -> data reg */
							sprintf(operand_s, "0x%04X(A%i),D%i", data, areg, dreg);
						} else {
							/* data reg -> mem */
							sprintf(operand_s, "D%i,0x%04X(A%i)", dreg, data, areg);
						}
						decoded = true;
					} break;
					case 51 : { /* MOVEQ */
						...
						sprintf(operand_s, "#0x%02X,D%i", (word & 0x00FF), dreg);
						decoded = true;
					} break;
					case 54 : /* NBCD */
					case 55 :
		...
		int fetched = address - start_address;
		if (!rawmode) {
			fetched = 4 - (fetched >> 1);
			while (fetched)
			{

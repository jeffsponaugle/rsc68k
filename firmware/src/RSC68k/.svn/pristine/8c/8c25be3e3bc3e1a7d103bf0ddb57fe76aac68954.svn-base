...
#include <sys/timeb.h>
#include <string.h>
#include "../Application/8086Tiny.h"
...
#define KEYBOARD_DRIVER read(0, mem + 0x4A6, 1) && (int8_asap = (mem[0x4A6] == 0x1B), pc_interrupt(7, mem))

// Keyboard driver for SDL
#define SDL_KEYBOARD_DRIVER KEYBOARD_DRIVER

// Global variable definitions
unsigned io_ports[IO_PORT_COUNT], *opcode_stream, *regs8, i_rm, i_w, i_reg, i_mod, i_mod_size, i_d, i_reg4bit, raw_opcode_id, xlat_opcode_id, extra, rep_mode, seg_override_en, rep_override_en, trap_flag, int8_asap, scratch_uchar, io_hi_lo, *vid_mem_base, spkr_en, bios_table_lookup[20][256];
...
int Tiny8086(CONTEXT8086 *psContext)
{
	int i,j;
	char *mem;
	...
	regs16 = (unsigned short *)(regs8 = (unsigned int *) (mem + REGS_BASE));
	regs16[REG_CS] = psContext->u16CS;
	reg_ip = psContext->u16IP;
	...
	for (; opcode_stream = (unsigned int *) (mem + 16 * regs16[REG_CS] + reg_ip), opcode_stream != (unsigned int *) mem;)
	{
		...
		switch (xlat_opcode_id)
		{
			...
			OPCODE 39: // INT imm8
				reg_ip += 2;
				pc_interrupt(i_data0, mem)
			OPCODE 40: // INTO
				++reg_ip;
				regs8[FLAG_OF] && pc_interrupt(4, mem)
			OPCODE 41: // AAM
				if (i_data0 &= 0xFF)

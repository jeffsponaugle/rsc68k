...
#define rdint(indxfile) \
    (ch = Zorkgetc(indxfile), \
     ((ch > 127) ? (ch - 256) : (ch)) * 256 + Zorkgetc(indxfile))
...
#else
    if ((dbfile = Zorkfopen(LOCALTEXTFILE, BINREAD)) == NULL &&
	(dbfile = Zorkfopen(TEXTFILE, BINREAD)) == NULL)
#endif
	goto L1950;
...
/* Save location of start of message text */
    rmsg_1.mrloc = Zorkftell(indxfile);

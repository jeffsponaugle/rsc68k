...
_vectorTable:
	...
	.long	BIOSEntry		/* 1   - PC startup vector */
...
SendHexLoopTHRE:
	moveb	(UART_REG_LSR << 1)(%a6), %d1
	andb	#UART_LSR_THRE, %d1
	beq	SendHexLoopTHRE

/* Send the byte */
	move.b	%d0, (UART_REG_RBRTHR << 1)(%a6)
	dbra	%d5, SendHexLoop
	rts
...
/* Let everyone know we've started execution of the BIOS */
	lea	RSC68KHW_DEVCOM_POST_LED, %a4
	POSTSet	POSTCODE_BIOS_START
...
/* Copy the vector table to the bottom of RAM */
	lea	_vectorTable, %a0
	lea	VECTOR_STACK_POINTER, %a1
	lea	_vectorTableEnd, %a2

/* Copy (a1)->(a0) until a0==a2 */

vectorTableCopyLoop:
	movel	(%a0)+, %d0
	movel	%d0, (%a1)+
	cmpl	%a0, %a2
	bne	vectorTableCopyLoop

/* Figure out what our stack pointer should be. When we're the supervisor CPU, top of
   stack is 0xf80000. Worker CPU top of stack is 0xff0000. Supervisor CPU is 0. */

/* Assume top of stack for supervisor CPU */

	lea	RSC68KHW_UPPER_RAM_END, %a0

/* If we're the supervisor, skip the worker CPU stack pointer. Look for the boot loader
   signature bytes in %d6/%d7. If it's there, then it's by definition the supervisor
   CPU since the worker CPUs don't have a boot loader */

	lea	g_u8SlotID, %a1
	moveb	(%a1), %d0
	cmpb	#SLOT_ID_SUPERVISOR, %d0
	beq	Supervisor

/* This is a worker CPU */

Worker:
	lea	RSC68KHW_DEVCOM_BASE, %a0

Supervisor:

/* Write it to the 68K's default stack pointer vector for later retrieval */
	movel	%a0, %sp
	lea	VECTOR_STACK_POINTER, %a0
	movel	%sp, (%a0)
	lea	_vectorTable, %a0
	movel	%sp, (%a0)
...
/* Signal that we're starting the UART init */
	...
	POSTSet	POSTCODE_BIOS_UART_A_INIT
...
/* Set the interrupt identification register with a 0. It should return UART_IIR_NOT_PENDING */
	...
	moveb	%d0, (UART_REG_IIR << 1) (%a6)

/* Now read the IIR back */

	moveb	(UART_REG_IIR << 1) (%a6), %d0
...
UARTTestLCR:
	moveb	#0xa5, %d0
	moveb	%d0, (UART_REG_LCR << 1)(%a6)
	moveb	(UART_REG_LCR << 1)(%a6), %d0
	cmpib	#0xa5, %d0
	beq	UARTTestLCR2
...
/* Initialize both UARTs to 115200kbps, 8 data bits, 1 stop bit, no FIFO, no interrupts */

/* Set baud rate */

UARTInit:
	lea	RSC68KHW_DEVCOM_UARTB, %a5

	moveb	#UART_LCR_DLAB_ENABLE, %d0
	moveb	%d0, (UART_REG_LCR << 1)(%a6)
	moveb	%d0, (UART_REG_LCR << 1)(%a5)

	moveb	#(BOOTLOADER_BAUD_RATE_DIVISOR & 0xff), %d0
	moveb	%d0, (UART_REG_RBRTHR << 1)(%a6)
	moveb	%d0, (UART_REG_RBRTHR << 1)(%a5)

	moveb	#(BOOTLOADER_BAUD_RATE_DIVISOR >> 8), %d0
	moveb	%d0, (UART_REG_IER << 1)(%a6)
	moveb	%d0, (UART_REG_IER << 1)(%a5)
...
/* Now assert DTR, RTS, OUT1 and OUT2 for both UART A and B */

	moveb	#UART_MCR_DTR + UART_MCR_RTS + UART_MCR_OUT1 + UART_MCR_OUT2, %d0
	moveb	%d0, (UART_REG_MCR << 1) (%a6)
	moveb	%d0, (UART_REG_MCR << 1) (%a5)

	POSTSet	POSTCODE_BIOS_RAM_CHECK

/* Fill RAM with 0x5aa5a55a */
...
fillLoop5aa5a55a:
	movel	%d3, (%a0)+
	cmpl	%a0, %a1
	bne	fillLoop5aa5a55a

/* Now do a readback and see what our pattern is */
...
checkLoop5aa5a55a:
	...
	bne	BIOSDRAMFault
	cmpl	%a0, %a1
	bne	checkLoop5aa5a55a
       
/* Now do a readback and see what our pattern is and fill with 0xffffffff */
...
checkLoopa55a5aa5:
	...
	bne	BIOSDRAMFault
	cmpl	%a0, %a1
	bne	checkLoopa55a5aa5

/* All ready to execute */

	POSTSet	POSTCODE_BIOS_MAIN_EXEC
	jmp	_start

DRAMFault:
	dc.b	0x0d 
	dc.b	0x0a
	.ascii	"BIOS DRAM fault at address 0x"
	dc.b	0x00
